...
# Configuración del bot con un pool de conexiones más grande
application = Application.builder().token("7866546786:AAF5irtu3cnjhTqvQ42qwNHF8WxRN8RSsC4").pool_timeout(120).build()

TOKEN = '7866546786:AAF5irtu3cnjhTqvQ42qwNHF8WxRN8RSsC4'
chat_id = '1668283863' 
...
async def send_message(text, chat_id):
    try:
        ...
        logger.info(f"Missatge enviat al chatID {chat_id}: {text}")
    except TelegramError as e:
        logger.error(f"Error en enviar el missatge: {e}")
    except Exception as e:
        logger.error(f"Error inesperat: {e}")

# Funciones para interactuar con las herramientas
async def send_large_message(text, chat_id):
    # Dividimos el mensaje en partes de 4096 caracteres
    while len(text) > 0:
        message_part = text[:4096]
        await send_message(message_part, chat_id)
        text = text[4096:]
        print(text)
        # Afegir un petit retras para evitar sobrecargar el servidor de Telegram
        await asyncio.sleep(3)  # Espera de 1 segon entre missatges
...
# Aumentar espaciado entre botones
    ...
    def mostrar_resultat(self, texto):
        ...
        asyncio.run_coroutine_threadsafe(send_large_message(texto_limpio, chat_id), loop)  
    ...
    async def enviar_resultat(self, ss):
        ...
        await send_large_message(ss, chat_id)
